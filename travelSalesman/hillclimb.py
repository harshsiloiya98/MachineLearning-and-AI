...
def generate2optNeighbours(tour):
    ...
    for i in range(2, cities - 1):
        for j in range(1, cities - i + 1):
            tour_dup = tour[:]
            tour_dup[j : j + i] = tour_dup[j : j + i][::-1]
            all_possible_neighbours.append(tour_dup[:])
    "*** --------------  ***"
        ...
        for j in range(i + 2, cities):
            for k in range(j + 2, cities):
                if (i == 0 and k == cities - 1):
                    continue
                tour1 = tour[0:i+1] + tour[i+1:j+1][::-1] + tour[j+1:k+1][::-1] + tour[k+1:]
                tour2 = tour[0:i+1] + tour[j+1:k+1] + tour[i+1:j+1] + tour[k+1:]
                tour3 = tour[0:i+1] + tour[j+1:k+1][::-1] + tour[i+1:j+1] + tour[k+1:]
                tour4 = tour[0:i+1] + tour[j+1:k+1] + tour[i+1:j+1][::-1] + tour[k+1:]
                if (tour1 not in all_possible_neighbours):
                    all_possible_neighbours.append(tour1)
                if (tour2 not in all_possible_neighbours):
                    all_possible_neighbours.append(tour2)
                if (tour3 not in all_possible_neighbours):
                    all_possible_neighbours.append(tour3)
                if (tour4 not in all_possible_neighbours):
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    l = getTourLength(initial_tour)
    while (True):
        tourLengthList.append(l)
        neighbours = getNeighbours(current)
        neighbours_l = list(map(getTourLength, neighbours))
        idx = neighbours_l.index(min(neighbours_l))
        if (neighbours_l[idx] > l):
            break
        current = neighbours[idx]
        l = neighbours_l[idx]
    print(tourLengthList)
    minTour.append(current)
    "*** --------------  ***"
    return tourLengthList, minTour
...
def nearestNeighbourTour(initial_city):
    ...
    while (len(tour) < cities):
        min_dist = float("inf")
        closest_city = 0
        current_city_node = nodeDict[current_city]
        for i, city in nodeDict.items():
            if (i == current_city or i in tour):
                continue
            dist = getDistance(city, current_city_node)
            if (dist < min_dist):
                min_dist = dist
                closest_city = i
...
def eucledianTour(initial_city):
    ...
    for i in range(1, cities + 1):
        for j in range(i + 1, cities + 1):
            city_i = nodeDict[i]
            city_j = nodeDict[j]
            edge = [i, j, getDistance(city_i, city_j)]
...
def finalOrder(mst, initial_city):
    ...
    while (not s.isEmpty()):
        current_city = s.pop()
        fin_order.append(current_city)
        for i, j in mst:
            if (i == current_city):
                if (j not in fin_order):
                    s.push(j)
            elif (j == current_city):
                if (i not in fin_order):
                    s.push(i)
    "*** --------------  ***"
    return fin_order
