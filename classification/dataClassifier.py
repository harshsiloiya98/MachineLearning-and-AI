...
def enhancedFeatureExtractorDigit(datum):
    ...
    # we add another feature based on the edges of the shapes
    # using sobel edge detection algorithm
    grad_rs = []
    grad = np.gradient(datum)
    for i in range(50):
        for j in range(50):
            grad_rs.append(np.sqrt(grad[i]**2 + grad[j]**2))
    for i in range(len(datum)):
        features[i] = datum[i]
    return features
