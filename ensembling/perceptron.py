...
class PerceptronClassifier:
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        n = int(0.7 * len(trainingData))
        index_sample = np.random.choice(range(len(trainingData)), size = n, replace = True, p = sample_weights)
        train_data_sample = [trainingData[index] for index in index_sample]
        train_label_sample = [trainingLabels[index] for index in index_sample]
        return train_data_sample, train_label_sample
