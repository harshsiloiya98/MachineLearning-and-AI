...
class AdaBoostClassifier:
    ...
    def train(self, trainingData, trainingLabels):
        ...
        for i in range(self.boosting_iterations):
            self.classifiers[i].train(trainingData, trainingLabels, sample_weights = weights)
            error = 0.0
            for j in range(n):
    ...
    def classify(self, data):
        ...
        guesses = []
        for datum in data:
            result = [self.classifiers[i].classify([datum])[0] for i in range(self.boosting_iterations)]
            guess = int(np.sign(np.sum(np.multiply(result, self.alphas))))
            if (guess == 0):
